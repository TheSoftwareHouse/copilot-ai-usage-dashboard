...
describe("POST /api/jobs/usage-collection", () => {
  ...
  it("returns 409 with skipped metadata when usage collection cannot start", async () => {
    await seedAuthSession();
    mockedExecuteUsageCollection.mockResolvedValueOnce({
      skipped: true,
      reason: "no_org_connected",
    });
    ...
    expect(json.reason).toBe("no_org_connected");
  });
  ...
  it("returns success payload when the job executes", async () => {
    ...
    expect(json.jobExecutionId).toBe(10);
    expect(json.status).toBe("success");
    expect(json.recordsProcessed).toBe(7);
