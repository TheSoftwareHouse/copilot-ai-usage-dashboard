...
export async function getTestDataSource(): Promise<DataSource> {
  ...
  testDataSource = new DataSource({
    ...
    entities: [ConfigurationEntity, CopilotSeatEntity, CopilotUsageEntity, JobExecutionEntity, UserEntity, SessionEntity, DashboardMonthlySummaryEntity, ImportHistoryEntity, TeamEntity, TeamMemberSnapshotEntity, DepartmentEntity, GitHubAppEntity],
    synchronize: true,
    logging: false,
  });
...
export async function cleanDatabase(ds: DataSource): Promise<void> {
  ...
  await ds.query('TRUNCATE TABLE import_history, team_member_snapshot, team, department, copilot_usage, copilot_seat CASCADE');
